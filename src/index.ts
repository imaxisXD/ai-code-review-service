...
const CONVEX_DEPLOYMENT_KEY = process.env.CONVEX_DEPLOYMENT_KEY as string;
...
functions.http('httpHandler', async (req, res) => {
  ...
  const { repoId, repoUrl, jobType, beforeSha, afterSha } = jobData;
  ...
  if (repoUrl.includes('github.com') && !GITHUB_TOKEN) {
  ...
  try {
    ...
    const repoGit = await gitService.cloneRepository(repoUrl, cloneDir, cloneOptions);
...
async function processFiles(
  ...
  for (let i = 0; i < filesToProcess.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (fullFilePath) => {
      try {
