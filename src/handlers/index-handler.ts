...
type FileProcessorServiceType = ReturnType<typeof createFileProcessorService>;
type GitServiceType = ReturnType<typeof createGitService>;
...
/**
 * Create an index handler for processing code repositories
 */
export function createIndexHandler(options: IndexHandlerOptions) {
   ...
   */
  async function handleIndexing(c: Context) {
    // Get the pre-validated request body from the auth middleware
    const jobData = c.get('requestBody') as IndexingJob;
  ...
  async function determineChanges(
    repoGit: ReturnType<ReturnType<typeof createGitService>['getSimpleGit']>,
    cloneDir: string,
    jobType: string,
    beforeSha: string,
    endSha: string,
